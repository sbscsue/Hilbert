...
void hilbertToC(const double x[3600], creal_T y[3600])
{
  static creal_T fv[8192];
  static creal_T fy[8192];
  static creal_T b_wwc[7199];
  static creal_T b_x[3600];
  ...
  for (k = 0; k < 1799; k++) {
    ...
    nt_im = -3.1415926535897931 * (double)rt / 1800.0;
    if (nt_im == 0.0) {
      nt_re = 1.0;
      nt_im = 0.0;
    } else {
  ...
  for (k = 1798; k >= 0; k--) {
    wwc[k + 1800] = wwc[1798 - k];
  }
  c_FFTImplementationCallback_doH(x, y, wwc, dv, sintabinv);
  for (i = 0; i < 1799; i++) {
    y[i + 1].re *= 2.0;
    y[i + 1].im *= 2.0;
    y[i + 1801].re = 0.0;
  ...
  memcpy(&b_x[0], &y[0], 3600U * sizeof(creal_T));
  rt = 0;
  b_wwc[3599].re = 1.0;
  b_wwc[3599].im = 0.0;
  for (k = 0; k < 3599; k++) {
    ...
    nt_im = 3.1415926535897931 * (double)rt / 3600.0;
    if (nt_im == 0.0) {
      nt_re = 1.0;
      nt_im = 0.0;
    } else {
    ...
    b_wwc[3598 - k].re = nt_re;
    b_wwc[3598 - k].im = -nt_im;
  }
  for (k = 3598; k >= 0; k--) {
    b_wwc[k + 3600] = b_wwc[3598 - k];
  }
  for (k = 0; k < 3600; k++) {
    nt_im = b_wwc[k + 3599].re;
    nt_re = b_x[k].im;
    twid_re = b_wwc[k + 3599].im;
    twid_im = b_x[k].re;
    y[k].re = nt_im * twid_im + twid_re * nt_re;
    y[k].im = nt_im * nt_re - twid_re * twid_im;
  }
  ...
  for (i = 0; i <= 8190; i += 2) {
    ...
    re_tmp = fv[i].re;
    nt_re = fv[i].im;
    fv[i + 1].re = re_tmp - nt_re_tmp;
    fv[i + 1].im = nt_re - nt_im;
    fv[i].re = re_tmp + nt_re_tmp;
    fv[i].im = nt_re + nt_im;
  }
  rt = 2;
  ju = 4;
  ...
  while (k > 0) {
    for (i = 0; i < iheight; i += ju) {
      b_nt_re_tmp = i + rt;
      nt_re = fv[b_nt_re_tmp].re;
      nt_im = fv[b_nt_re_tmp].im;
      fv[b_nt_re_tmp].re = fv[i].re - nt_re;
      fv[b_nt_re_tmp].im = fv[i].im - nt_im;
      fv[i].re += nt_re;
      fv[i].im += nt_im;
    }
    istart = 1;
    for (j = k; j < 4096; j += k) {
      ...
      while (i < ihi) {
        b_nt_re_tmp = i + rt;
        nt_re_tmp = fv[b_nt_re_tmp].im;
        nt_im = fv[b_nt_re_tmp].re;
        nt_re = twid_re * nt_im - twid_im * nt_re_tmp;
        nt_im = twid_re * nt_re_tmp + twid_im * nt_im;
        fv[b_nt_re_tmp].re = fv[i].re - nt_re;
        fv[b_nt_re_tmp].im = fv[i].im - nt_im;
  ...
  for (rt = 0; rt < 8192; rt++) {
    ...
    twid_re = fy[rt].im;
    twid_im = fv[rt].re;
    fy[rt].re = re_tmp * twid_im - twid_re * nt_im;
    fy[rt].im = re_tmp * nt_im + twid_re * twid_im;
  }
  rt = 0;
  ju = 0;
  for (i = 0; i < 8191; i++) {
  ...
  for (k = 0; k < 3600; k++) {
    ...
    nt_re = fv[k + 3599].im;
    twid_re = b_wwc[k + 3599].im;
    twid_im = fv[k + 3599].re;
    re_tmp = nt_im * twid_im + twid_re * nt_re;
    nt_im = nt_im * nt_re - twid_re * twid_im;
    if (nt_im == 0.0) {
      y[k].re = re_tmp / 3600.0;
      y[k].im = 0.0;
    } else if (re_tmp == 0.0) {
      y[k].re = 0.0;
      y[k].im = nt_im / 3600.0;
...
/* End of code generation (hilbertToC.c) */
